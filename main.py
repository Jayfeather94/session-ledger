...
数据层在 session_core.py，原样搬过来用，一行没改。

和 tkinter 版最大的区别：**半透明是 Qt 的原生能力**。
列表、搜索框、按钮都设成 rgba 半透明色，Qt 自己在合成时把它们和底下的图片混起来 ——
不需要背后再开一个窗口，不需要颜色键，不需要同步，也就没有「追不上」的僵硬感。

（tkinter 版 会话.py 还留着，随时能退回去。）
"""
